module Polycon
  class Appointment
    ...
    def self.export_week_for_professional professional, date
      date_parsed = beggining_of_week date
      days = []
      data = []
      for i in 0..6
      ...
      puts "Export from #{professional} in week #{date} was created in #{exports_dir}/#{professional}"
    end
    ...
    def self.export_week date  
      ...
      puts "Export from #{date} was created in #{exports_dir}"
    end
    ...
    def self.create_html_for_professional_in_date data, headers, professional
      ...
      FileUtils.mkdir_p("#{Dir.home}/.polyconExports/#{professional}") unless Dir.exists?("#{Dir.home}/.polyconExports/#{professional}")
      File.open("#{exports_dir}/#{professional}/export.html", "w") do |f|     
          f.write(html) 
          f.close
        end
    end
    ...
    def self.create_html_in_date data, headers, date
      ...
      name = "Export" + date
      File.open("#{exports_dir}/exports_by_date/#{name}.html", "w") do |f|     
          f.write(html) 
          f.close
      end
    end
